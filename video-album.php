...
if ($dilwrite['dil_id']==1) {
	...
	$metakey=$db->prepare("SELECT * from meta2 where meta_id=7");
} elseif ($dilwrite['dil_id']==3) {
	$metakey=$db->prepare("SELECT * from meta3 where meta_id=7");
}
...
<div class="breadcrumb breadcrumb-1 pos-center" style="background: url(trex/<?php echo $settingsprint['ayar_title5']; ?>) no-repeat fixed; width: 100%;">
	<h1><?php echo $widgetprint['widget_ceviri2']; ?></h1>
</div>
		...
		<div class="container">
			<div class="row">	
				...
				while ($picprint=$picsor->fetch(PDO::FETCH_ASSOC)) {
					...
					if ($dilwrite['dil_id']==1) {
						$LinkAlbum = seo('video-album-detay-'.$picprint["kategori_ad"]).'-'.$picprint["kategori_id"];
						$AlbumAd = $picprint['kategori_ad'];
					} elseif ($dilwrite['dil_id']==2) {
						$LinkAlbum = seo('video-album-detay-'.$picprint["kategori_ad2"]).'-'.$picprint["kategori_id"];
						$AlbumAd = $picprint['kategori_ad2'];
					} elseif ($dilwrite['dil_id']==3) {
						$LinkAlbum = seo('video-album-detay-'.$picprint["kategori_ad3"]).'-'.$picprint["kategori_id"];
